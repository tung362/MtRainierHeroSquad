...
class JulioBody:
    ...
    def OnLogicUpdate(self, UpdateEvent):
        ...
        if(Parent.JulioPlayerController.Side is True):
            
            self.Owner.Orientation.LookAtPoint(Vector3(WorldMousePosition.x, WorldMousePosition.y, 0))
            
        if(Parent.JulioPlayerController.Side2 is True):
            self.Owner.Orientation.LookAtPoint(Vector3(WorldMousePosition.x, WorldMousePosition.y, 0))
        ...
        if(Zero.Keyboard.KeyIsDown(Zero.Keys.S) and not Zero.Keyboard.KeyIsDown(Zero.Keys.A) and not Zero.Keyboard.KeyIsDown(Zero.Keys.D)):
            self.Owner.Transform.Translation = Vector3(self.BodyX, self.BodyY - 0.06, self.BodyZ)
            Parent.BoxCollider.Offset = Vector3(0.02, 0.07, 0)
            Parent.BoxCollider.Size = Vector3(0.1, 0.3, 10)
            
        elif(Zero.Keyboard.KeyIsDown(Zero.Keys.Space) or Parent.JulioPlayerController.OnGround is False):
...
Zero.RegisterComponent("JulioBody", JulioBody)