    ...
    def OnLogicUpdate(self, UpdateEvent):
        ...
        if(Zero.Mouse.IsButtonDown(Zero.MouseButtons.Left)):
            ...
            if(self.timer > self.Delay):
                ...
                Bullet = self.Space.CreateAtPosition("PlayerMissle", Vector3(WorldPosition.x, WorldPosition.y, ObjectPostitionZ))
                Bullet.Transform.Rotation = Parent.Transform.WorldRotation
                Bullet.PlayerMissle.FireY = self.RandomAngle
        ...
        if(self.CanShoot is True):
            Parent.Sprite.SpriteSource = "TreyBodyShoot"
            self.CanShoot = False
        elif(self.CanIdle is True):
            Parent.Sprite.SpriteSource = "TreyBody"
            self.CanIdle = False

Zero.RegisterComponent("MissleGun1", MissleGun1)