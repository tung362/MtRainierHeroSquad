...
class PlayerGiantBullet:
    ...
    def OnLogicUpdate(self, UpdateEvent):
        ...
        if(self.Start == True):
            ...
            #Name
            self.Owner.Name = "PlayerGiantBullet"
    ...
    def OnCollisionStarted(self, CollisionEvent):
        ...
        if(other.Name == "GunMan"):
            ...
            self.Owner.SoundEmitter.PlayCue("PlayerHitBullet")
            self.HasCollided = True
            self.Owner.Sprite.SpriteSource = "GiantBulletHit"
            self.Owner.RemoveComponentByName("RigidBody")
            Action.Delay(sequence, 0.4)
            Action.Call(sequence, self.OnDeath)
            
        elif(other.Name == "HeavyGunMan"):
            
            self.Owner.SoundEmitter.Volume = 0.4
            self.Owner.SoundEmitter.PlayCue("PlayerHitBullet")
            self.HasCollided = True
            self.Owner.Sprite.SpriteSource = "BulletHit"
            self.Owner.RemoveComponentByName("RigidBody")
            Action.Delay(sequence, 0.4)
            Action.Call(sequence, self.OnDeath)
...
Zero.RegisterComponent("PlayerGiantBullet", PlayerGiantBullet)