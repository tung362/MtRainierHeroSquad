...
class PeytonBody:
    ...
    def OnLogicUpdate(self, UpdateEvent):
        ...
        if(Parent.PeytonPlayerController.Side is True):
            
            self.Owner.Orientation.LookAtPoint(Vector3(WorldMousePosition.x, WorldMousePosition.y, 0))
            
        if(Parent.PeytonPlayerController.Side2 is True):
            self.Owner.Orientation.LookAtPoint(Vector3(WorldMousePosition.x, WorldMousePosition.y, 0))
        ...
        elif(Zero.Keyboard.KeyIsDown(Zero.Keys.Space) or Parent.PeytonPlayerController.OnGround is False):
...
Zero.RegisterComponent("PeytonBody", PeytonBody)