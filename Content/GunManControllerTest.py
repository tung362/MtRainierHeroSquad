...
class GunManControllerTest:
    ...
    def ApplyMovement(self, UpdateEvent):
        if(self.AICanMove is True):
            MoveDirection = Vector3(0, 0, 0)
            
            TungBody = self.Space.FindObjectByName("GunManBody")
            ...
            if(Tung):
                ...
                Distance = GetTungPos - self.Owner.Transform.Translation
                
            elif(Austin):
                GetAustinPos = Austin.Transform.Translation
                Distance = GetAustinPos - self.Owner.Transform.Translation
                
            elif(Trey):
                GetTreyPos = Trey.Transform.Translation
                Distance = GetTreyPos - self.Owner.Transform.Translation
                
            elif(Peyton):
                GetPeytonPos = Peyton.Transform.Translation
                Distance = GetPeytonPos - self.Owner.Transform.Translation
                
            elif(Sedrick):
                GetSedrickPos = Sedrick.Transform.Translation
                Distance = GetSedrickPos - self.Owner.Transform.Translation
                
            elif(Julio):
                GetJulioPos = Julio.Transform.Translation
                Distance = GetJulioPos - self.Owner.Transform.Translation
                
                
                
            if(self.Start is True):
                self.RandomAction = random.randint(1,4)
                self.RandomTime = random.uniform(0.1, 1.0)
                self.Timer = 0
                print("fawfw")
                self.Start = False
